 ...
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 */
import { StoryType, type GeneratedStoryConfig, type StoryMetaType } from '@leafygreen-ui/lib';
...
import { generatedStoryWrapper } from './prop-combinations.styles';
import { isGeneratedStory } from './utils';

type ContextType<T extends React.ComponentType<any>> = StoryContext<T> & StoryMetaType<T>;
 ...
 */
export const PropCombinationsDecorator: Decorator = (
  StoryFn: StoryFn,
  ...
  if (component) {
    const { parameters, args: metaArgs } = context as ContextType<typeof component>;

    const decoratorConfig: GeneratedStoryConfig<typeof component> | undefined =
      parameters[PARAM_NAME];

    if (!isUndefined(decoratorConfig)) {
      const { combineArgs, excludeCombinations, args: generatedArgs, decorator } = decoratorConfig;
      if (isGeneratedStory(context)) {
        ...
        const GeneratedScreen: StoryType<typeof component> = () => (
          <div className={generatedStoryWrapper}>
            <PropCombinations
              component={component}
              variables={variables}
