import { type ExtendedComponentProps, type GeneratedStoryConfig } from '@leafygreen-ui/lib';
import { entries, has, isEqual, isUndefined, keys } from 'lodash';
import { ComponentType, ElementType } from 'react';

export function shouldExcludePropCombo<T extends ComponentType<any>>({
  propName,
  val,
  props,
  exclude,
    ...
    if (Array.isArray(rule)) {
      if (rule.every((x) => typeof x === 'string')) {
        ...
        if (rule.includes(propName) && !isUndefined(val)) {
          ...
          return rule.some(
            (p) => keys(props).includes(p as string) && !isUndefined(props[p as string]),
          );
        } else {
          return false;
        }
        ...
        if (
          ...
          console.warn('Invalid condition in `excludeCombinations` definition', rule);
          return false;
        }
        ...
        } else if (keys(props).includes(checkProp as string) && !isUndefined(props[checkProp])) {
          // @ts-ignore
      ...
      if (has(rule, propName) && doValuesMatch(val, rule[propName])) {
        ...
        return entries(rule).every(([name, conditionValue]) => {
          return name === propName || isConditionInProps(props, name, conditionValue);
        });
      }
    }
  });
 ...
 */
function areValuesEqual<T extends ElementType<any>>(
  //@ts-ignore
  ...
  //@ts-ignore
  if (typeof v1 === 'object' && typeof v2 === 'object' && has(v1, 'type') && has(v2, 'type')) {
    //@ts-ignore
...
function doValuesMatch<T extends ElementType<any>>(
  //@ts-ignore
...
function isConditionInProps<T extends ElementType<any>>(
  props: Record<string, any>,
  key: string,
  //@ts-ignore
